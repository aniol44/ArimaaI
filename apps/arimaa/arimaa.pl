...
/%
tout_deplacement_possible_silver(Board, TempBoard, Res) --> Res sunifie avec une liste de tous les deplacements possibles du joueur silver avec une profondeur de 1.
...
deplacement_possible(Board, Deplacement ) --> renvoie vrai si le deplacement du pion dune case à une autre case voisine (précisée) est possible sinon renvoie faux (la profondeur du deplacement est de 1 case).

Pour linstant pas de cut (!), Peut renvoyer tout les déplacements par exemple on appel sur deplacement(Board, [[5,5], [X,Y]]. A voir après avec l\utilisation de Setof (+ retract et asserta).
...
voisins(Board, Coord, Res) --> Res sunifie avec une liste des voisins [VoisinH, VoisinB, VoisinG, VoisinD] Avec VoisinH pouvant être : [X,Y,piece,joueur], [X,Y,-1,-1] ou [] (quand hors du plateau). 
...
/*get_case(Board, Coord, Res): Res sunifie avec le contenu dune case en fonction dune coordonnée. Contenu possible:
 - [X,Y,Piece,Joueur] --> Cas dune pièce.
 - [X,Y,-1,-1] --> cas dune case vide. (attention, dans tous les cas Res peut sunifier avec [X,Y,-1,-1] si X et Y sont des coords dans le plateau, donc toujours appeler get_case avec un Res : [X,Y,A,B] puis tester A, B)
 - False --> Hors plateau*/
...
/*deplacement(Board,NvBoard,Depart,Arrive) :- NvBoard sunifie avec Board modifié, on modifie seulement les coordonnées dune pièce sans verification.*/
